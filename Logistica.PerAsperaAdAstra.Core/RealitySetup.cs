...
public record RecipeDto(RecipeCategory Category, List<RecipeInputDto> Inputs, List<RecipeOutputDto> Outputs, bool IsPrimaryMassDefinition = false);
...
public sealed record ElectricMotorSpecification(double MaxPowerKw) : IComponentSpecification;
public sealed record CombustionEngineSpecification(double MaxPowerKw, string FuelType, double Efficiency) : IComponentSpecification;
public sealed record RocketEngineSpecification(double MassFlowRateKgS, double ExhaustVelocityMps, double ExhaustPressurePa, double NozzleAreaM2) : IComponentSpecification;
...
public class RealitySetup
{
    ...
    public RealitySetup(string? dataPath = null)
    {
    ...
    private void PopulateDtosFromDefaults()
    {
        ...
        [
            ...
            Specifications:[new ElectricMotorSpecification(MaxPowerKw:1200)], Tags:["engine", "electric"])
        ];
